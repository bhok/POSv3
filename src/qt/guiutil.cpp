...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("swagbucks"))
        return false;
...
{
    // Convert swagbucks:// to swagbucks:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("swagbucks://"))
    {
        uri.replace(0, 12, "swagbucks:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SwagBucks.lnk";
}
...
{
    return GetAutostartDir() / "swagbucks.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SwagBucks-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  swagbucks-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("SwagBucks-Qt"));
